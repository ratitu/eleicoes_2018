...
st.set_page_config(page_title="Mapa Eleições 2018 - 2º Turno", layout="wide")

st.title("Mapa do resultado do 2º Turno das Eleições Presidenciais de 2018 por Município")
st.markdown("---")
...
def load_data():
    ...
    df_votacao['RESULTADO'] = (df_votacao['NR_CANDIDATO'] == 17)
...
def merge_data(_df_mapa, _df_votacao, _df_equivalencia):
    ...
    df_mapa_novo = _df_mapa.set_index('CD_GEOCMU').join(
...
with col2:
    vitorias_bolsonaro = df_filtered[df_filtered['NR_CANDIDATO'] == 17]['CD_MUNICIPIO'].nunique()
    st.metric("Vitórias de Bolsonaro", vitorias_bolsonaro)
with col3:
    vitorias_haddad = df_filtered[df_filtered['NR_CANDIDATO'] == 13]['CD_MUNICIPIO'].nunique()
    st.metric("Vitórias de Haddad", vitorias_haddad)
...
<div style="position: fixed; bottom: 30px; right: 30px; z-index: 1000;
    ...
    <p style="margin: 0;"><span style="background:{col_haddad}; width:12px; height:12px; display:inline-block; margin-right:6px;"></span>Fernando Haddad</p>
